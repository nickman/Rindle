...
/**
 * <p>Title: PerfHashTest</p>
 * <p>Description: </p> 
 * <p>Company: Helios Development Group LLC</p>
 * @author Whitehead (nwhitehead AT heliosdev DOT org)
 * <p><code>benchmarks.maps.PerfHashTest</code></p>
 */
...
public class PerfHashTest extends Thread {
  static int _read_ratio, _gr, _pr;
  static int _thread_min, _thread_max, _thread_incr;
  static int _table_size;
  static int _map_impl;

  static ConcurrentMap<Long,String> make_map( int impl ) {
    switch( impl ) {
    case 1: return new ConcurrentGuavaMap(CacheBuilder.newBuilder()
    		.initialCapacity(16)
    		.concurrencyLevel(16)
    		.recordStats()
    		.build()
    		);
    case 2: return new ConcurrentGuavaMap(CacheBuilder.newBuilder()
    		.initialCapacity(16)
    		.concurrencyLevel(256)
    		.build()
    		);
    case 3: return new ConcurrentHashMap<Long,String>(16,0.75f,  16); // force to   16 striping
    case 4: return new ConcurrentHashMap<Long,String>(16,0.75f, 256); // force to  256 striping
    case 5: return new ConcurrentHashMap<Long,String>(16,0.75f,4096); // force to 4096 striping
    case 6: return new NonBlockingHashMap<Long,String>(1024);
    case 7: return new NonBlockingHashMapLong<String>(1024);
...
//      _map_impl     = check( args[5], "implementation", -1, names.length );
        _read_ratio   = check( 100, "read%", 0, 100 );
        _thread_min   = check( 1, "thread_min", 1, 100000 );
        _thread_max   = check( 2, "thread_max", 1, 100000 );
        _thread_incr  = check( 1, "thread_incr", 1, 100000 );
        _table_size   = check( 100000, "table_size", 1, 100000000 );
        _map_impl     = check( 7, "implementation", -1, names.length );
    ...
    KEYS = new long[keymax];
    VALUES = new String[keymax];
    int [] histo = new int[64];
    for( int i=0; i<KEYS.length; i++ ) {
    	KEYS[i] = i;
      VALUES[i] = String.valueOf(i) + "abc" + String.valueOf(i*17+123);
      VALUES[i].intern();
      histo[VALUES[i].hashCode() >>>(32-6)]++;
    }
  ...
  static void run_till_stable( int num_threads, int num_trials, int impl ) throws Exception {
    ConcurrentMap<Long,String> HM = make_map(impl);
    if( HM == null ) return;
...
//      }
  ...
  final int _tnum;
  final ConcurrentMap<Long,String> _hash; // Shared hashtable
  final long[] _ops;
  final long[] _nanos;
  PerfHashTest( int tnum, ConcurrentMap<Long,String> HM, long[] ops, long[] nanos ) { _tnum = tnum; _hash = HM; _ops = ops; _nanos = nanos; }

  static long run_once( int num_threads, ConcurrentMap<Long,String> HM, long[] ops, long[] nanos ) throws Exception {
    Random R = new Random();
    _start = false;
    _stop = false;
    ...
    while( sz < ((_table_size>>1)+(_table_size>>3)) ) {
      ...
      while( true ) {
          ...
          String value = VALUES[index];
        if( sz < _table_size ) {
          if( HM.put(key,value) == null ) { sz++; break; }
        } else {
    ...
    // Launch threads
    PerfHashTest thrs[] = new PerfHashTest[num_threads];
    for( int i=0; i<num_threads; i++ )
      thrs[i] = new PerfHashTest(i, HM, ops, nanos);
    for( int i=0; i<num_threads; i++ )
      thrs[i].start();
    // Run threads
  ...
  public void run() {
    ...
    if( _read_ratio == 0 ) {
      ...
      if( _hash instanceof ConcurrentHashMap<?, ?> ) total = run_normal( (ConcurrentHashMap<Long, String>) _hash);
      else total = run_normal((ConcurrentMap<Long, String>) _hash);
    }
  ...
  public int run_churn() {
    ...
    while( !_stop ) {
      ...
      long key1 = KEYS[(idx+reprobe*10) & (KEYS.length-1)];
      _hash.put(key1,"" + key1);
      put_ops++;

      // Remove a key  0 probes in the future
      long key2 = KEYS[(idx+reprobe* 0) & (KEYS.length-1)];
      _hash.remove(key2);
      del_ops++;
...
//  }
  ...
  public int run_normal( ConcurrentHashMap<Long,String> hm ) {
    SimpleRandom R = new SimpleRandom();
  ...
  public int run_normal( ConcurrentMap<Long,String> hm ) {
    SimpleRandom R = new SimpleRandom();
    ...
    while( !_stop ) {
      ...
      if( x < _gr ) {
        ...
        if( val != null && !val.equals(value) ) { 
          throw new IllegalArgumentException("Mismatched key="+key+" and val="+val + " and value=" + value);
        }
      } else if( x < _pr ) {
        put_ops++;
    hm.putIfAbsent( key, value );
